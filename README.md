...
| Correção, aprovação e relatório | Cada execução é salva. Uma sugestão reprovada pode ser corrigida por regra determinística (sem chamar o modelo) ou editada à mão, com o original e cada mudança registrados; editar cancela a aprovação. Uma pessoa aprova (só se todas as verificações passaram), baixa o mapeamento em CSV e gera o relatório AS-IS do dataset, pronto para PDF | Disponível (web) |
